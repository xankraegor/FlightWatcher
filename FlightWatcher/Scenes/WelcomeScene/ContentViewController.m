...
- (instancetype)init {
    self = [super init];
    if (self) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width / 2 - 100.0,
                [UIScreen mainScreen].bounds.size.height / 2 - 100.0, 200.0, 200.0)];
...
float heightForText(NSString *text, UIFont *font, float width) {
    if (text) {
        ...
        return ceilf((float) needLabel.size.height);
    }
    return 0.0;
}
